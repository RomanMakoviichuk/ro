...
function getPrice(element, className) {
  ...
  return parseFloat(priceText.replace('LEI', '').replace(',', '.'));
}

// formating price to neccessary format
function formatPrice(price) {
  return price.toFixed(0).replace('.', ',') + ' LEI';
}
...
function handleRadioButtonChange(radioButton) {
  ...
  totalDiscount.textContent = 'Economisiti ' + formatPrice(oldPrice - actualPrice);

  // update link for selected package
  if(Number(selectedPackage) === 3) {
    addToCartLink.href = 'https://swisspower.ro/cart/48478955405637:3';
    totalOldPrice.classList.remove('hide')
    totalDiscount.classList.remove('hide')
  } else if (Number(selectedPackage) === 1) {
    addToCartLink.href = 'https://swisspower.ro/cart/48478955405637:1';
    totalOldPrice.classList.remove('hide')
    totalDiscount.classList.remove('hide')
  } else {
    addToCartLink.href = 'https://swisspower.ro/cart/48478955405637:' + selectedPackage;
    totalOldPrice.classList.remove('hide')
    totalDiscount.classList.remove('hide')
  }
